"""
Mesosfer Bear AI - Stage 3: Supervised Fine-Tuning (SFT) CLI

Chat/instruction tuning on conversation and instruction-following data.
Low LR to preserve pretrained knowledge. Requires CPT checkpoint.

Usage:
  uv run python -m train.sft --resume storage/models/bear_final.pt
  uv run python -m train.sft --resume storage/models/bear_final.pt --max-steps 8000
  uv run python -m train.sft --dry-run
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Mesosfer Bear AI - Stage 3: Supervised Fine-Tuning (SFT)",
    ...
    # Training — SFT defaults: low LR, light weight decay
