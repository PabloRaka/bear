"""
Mesosfer Bear AI - Stage 4: Safety Alignment CLI

Refusal and guardrail training on harmful-content rejection data.
Minimal LR and steps to avoid catastrophic forgetting. Requires SFT checkpoint.

Usage:
  uv run python -m train.safety --resume storage/models/bear_final.pt
  uv run python -m train.safety --resume storage/models/bear_final.pt --max-steps 3000
  uv run python -m train.safety --dry-run
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Mesosfer Bear AI - Stage 4: Safety Alignment",
    ...
    # Training — Safety defaults: minimal LR to prevent catastrophic forgetting
    ...
    parser.add_argument("--checkpoint-dir", type=str, default="storage/models/safety")
    ...
    parser.add_argument("--resume", type=str, default=None, help="Path to SFT checkpoint (defaults to storage/models/sft/bear_final.pt)")
    ...
    if not resume_from:
        for candidate in ("storage/models/sft/bear_final.pt", "storage/models/bear_final.pt"):
            if Path(candidate).exists():
                resume_from = candidate
                break
    if not resume_from and not args.dry_run:
        print("  [ERROR] SFT checkpoint not found! Please specify --resume storage/models/sft/bear_final.pt")
        import sys
        sys.exit(1)
