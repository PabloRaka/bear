"""
Mesosfer Bear AI - Stage 2: Continual Pre-training (CPT) CLI

Domain-specialized continued training on code, math, and terminal data.
Lower LR than pretrain, shorter warmup. Requires pretrain checkpoint.

Usage:
  uv run python -m train.cpt --resume storage/models/bear_final.pt
  uv run python -m train.cpt --resume storage/models/bear_final.pt --max-steps 30000
  uv run python -m train.cpt --dry-run
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Mesosfer Bear AI - Stage 2: Continual Pre-training (CPT)",
    ...
    # Training — CPT defaults: lower LR, shorter warmup
    ...
    if args.dry_run:
        ...
        args.log_every = 1
        args.dtype = "float32"
        args.resume = None  # no checkpoint needed for dry-run
